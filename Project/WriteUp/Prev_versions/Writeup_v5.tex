...
\usepackage{fancyhdr}
\pagestyle{fancy}
\fancyhead[L]{\rightmark} %  sets the right head element 
\fancyhead[R]{}
...
\begin{itemize}
	...
	\item The gamma distribution in the agri-rural environment suggests variations in colony movement are asymmetrically constrained around a mean, optimal foraging distance. This suggests foraging distances are more constrained towards shorter distances and longer ones are more permitted.
	\item The half-normal distribution in the urban environment indicates a lighter tail. This suggests foraging distances are more constrained for longer distances. This is reinforced by the significant difference in foraging distance between these two environments.
	\item These results suggest factors of the environment serve to constrain variations in worker foraging distances. However, these mechanisms are speculative and further studies are needed to test these hypothesis. However, our study highlights the importance of evaluating ecological patterns phenomenologically in order to drive the development of more targeted mechanistic models.
...
Given the different spatio-temporal distributions of pollen and nectar resources, combined with the different skills needed to harvest them, it has been proposed that forager specialisation provides the most efficient strategy \citep{Duarte2012}. This is reinforced by studies that have shown pollen and nectar foragers show different sucrose response thresholds \citep{Pankiw2000} and differ in brain neuropeptide profiles \citep{Brockmann2009}. Despite this, that our results show all foraging occurred at a single scale indicates the distribution of these resources in the environment is intermixed. Further, this observation that scale did not vary with time also indicates floral resource availability remained constant over the season. However, due to the shortage of data points at each month it was not feasible to explore scale within months using our method. As our observations were recorded bimonthly we have but a snap shot of foraging behaviour over time which may not be representative of the colony foraging behaviour during this time. Future studies should increase data collection over time to improve this resolution.

Constrained variation in colony foraging distances may also arise from environmental features. Differential types and distributions of resources in different environments could force an optimal strategy of reduced variation. As our data comes from different environments the fits of other distributions provides additional information on the role of landscape structure in creating our observed trends in scale. In both environments, the tail of the distributions follows a single exponential as the most parsimonious model. For the full distributions the best fitting functions differ in their tail properties between environments. The agri-rural environment shows the most likely distribution is the gamma, whereas the urban data is best described by a half-normal distribution. The dominance of these alternate distributions suggests aspects of these different landscapes constrain foraging distances differently.

The gamma distribution with shape $n$ and scale $\theta$ is equivalent to a sum of $n$ exponential random variables with a mean rate of $\theta$. Under laws of maximum entropy, the gamma, like the exponential and Poisson distributions, is often associated with neutral or random processes as they represent aggregates of process occurring at smaller scales which balance out \citep{Frank2009}. However, the gamma distribution has a heavier tail than the exponential. In biological terms this indicates variations in individual foraging distances congregate asymmetrically around a mean value, suggesting variation is constrained towards a mean over the colony but that increased foraging distances are less restricted than those closer to the hive. The half-normals dominance in the urban data describes a similar process of variations around some mean value but has a lighter tail. This property suggests the range of possible values is more restricted in the urban environment. 

Whilst the distributions are significantly different between these environments its possible this is also an artefact of low sample size. With more data it is possible the half-normal distribution could shift more towards the gamma, which is the next best fitting model. The presence of the gamma in both environments would indicate asymmetric variance, however, if the respective parameters were significantly different this could still indicate different possible ranges of values leading to the same inferences. The dominance of these distributions at different locations raises questions as to the underlying mechanisms maintaining uni-scalar movement. As the methods presented herein are phenomenological, inferences about possible mechanisms are purely speculative. Nevertheless, our results can help guide the directions of future studies investigating mechanisms. 

The parsimony of the half-normal distribution in the urban environment is possibly the result of greater physical barriers, such as buildings, than are present in the relatively open spaces of agri-rural settings. Isolated to a green oasis, the urban colony is potentially limited to forage within a certain range. However, green urban spaces such as parks, allotments and gardens can provide high floral resources throughout the season \citep{Baldock2015, Baldock2019, Plascencia2017}, indicating forage availability is sufficient in these environments to permit shorter foraging distances. Both of these mechanisms could produce a distribution with a lighter tail. The exact mechanism could be tested in a future study by providing a colony of a given size with enough resources to grow at a given distance. Then, by varying the number of obstacles, the effects could be compared. [example of this process in other studies?]

In an agri-rural setting, floral availability is limited by the presence of crops, many of which offer few resources to honey bees. With crop and pasture land taking up most of the area, resources for pollinators are often restricted to hedge rows and small verge patches. This limit in foraging area may homogenise the optimal foraging locations between individuals, causing the colony as a whole to forage at specific patches. The lack of barriers, such as buildings, combined with the presence of larger areas between resources in the agri-rural area may relax the constraints observed in the urban environment, causing the range of possible values to increase, as seen by the parsimony of the heavier tailed gamma distribution. [Need example of similar study].

\subsection{Limitations (remove and integrate into main body)}

\hspace{\parindent}
Overall, our results indicate that despite the presence of around 30,000 individuals honey bee colonies forage along the same scale. That these scales differed significantly between environments, combined with the parsimony of distributions with different tail properties, suggests environmental factors play a key role in constraining worker foraging variation. However, as a phenomenological study, the inference of mechanisms is largely speculative. In addition, the sample size of our data imposes limitations on these findings. Whilst our SumExp method identified a single scale in foraging distances our tests with synthetic data indicated large sample sizes are needed to pinpoint the true underlying scale value. This is further confounded by the issues associated with Gaussian characteristics. 

In our fitting of distributions to foraging distances in each environment the dominance of distributions with differing tail properties was used to suggest foraging behaviour is more constrained in urban than agri-rural environments. Whilst the distributions are significantly different between these environments its possible this is also an artefact of low sample size. With more data it is possible the half-normal distribution could shift more towards the gamma, which is the next best fitting model. The presence of the gamma in both environments would indicate asymmetric variance, however, if the respective parameters were significantly different this could still indicate different possible ranges of values. As such the inferences would remain the unchanged. It is suggested these observations could be due to more barriers in the urban environment. This is speculation as the study has no data on the presence of obstacles and works off the assumption that urban areas have more roads, buildings and other structures repellent to pollinators. A more mechanistic study is needed to determine this. 

\subsection{Concluding remarks (integrate into main body)}
